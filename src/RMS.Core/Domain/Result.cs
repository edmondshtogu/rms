...
namespace RMS.Core.Domain
{
    internal class ResultCommonLogic<TError>
    {
        ...
        public void SetObjectData(SerializationInfo oInfo, StreamingContext oContext)
        {
            IsFailure = oInfo.GetBoolean("IsFailure");
            Error = (TError)oInfo.GetValue("Error", typeof(TError));
        }
    }
    ...
    public struct Result : IResult, ISerializable
    {
        ...
        {
            return new Result<TValue, TError>(false, value, default);
        }

        [DebuggerStepThrough]
        public static Result<TValue, TError> Fail<TValue, TError>(TError error) where TError : class
        {
            return new Result<TValue, TError>(true, default, error);
        }
